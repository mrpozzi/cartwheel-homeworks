...
from observability.instrument import flush_workshop, load_env, setup_tracing, setup_workshop
...
def create_session(body: SessionCreate) -> dict[str, Any]:
    ...
    if user.role != body.role:
        raise HTTPException(
            status_code=403, detail="requested role does not match the stored role"
        )
...
async def post_message(
    ...
    version = prompt_version()  # hashes the template, before user context is injected
